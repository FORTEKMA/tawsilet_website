...
const OrderStatusStepper = ({ commandStatus }) => {
  ...
  if (commandStatus === "Failed_delivery") {
    return (
      <div className={classes.root} dir={isRTL ? "rtl" : "ltr"}>
        <Typography variant="h6" className={classes.errorStep}>
          {t("ClientProfile.Commande.status.failed_delivery")}
        </Typography>
        <Typography className={classes.stepDescription}>
          {t("ClientProfile.Commande.Message.failed_delivery")}
        </Typography>
      </div>
    );
  } else if (commandStatus === "Failed_pickup") {
    return (
      <div className={classes.root} dir={isRTL ? "rtl" : "ltr"}>
        <Typography variant="h6" className={classes.errorStep}>
          {t("ClientProfile.Commande.status.failed_pickup")}
        </Typography>
        <Typography className={classes.stepDescription}>
          {t("ClientProfile.Commande.Message.failed_pickup")}
        </Typography>
      </div>
    );
  } else if (commandStatus === "Canceled_by_partner") {
    return (
      <div className={classes.root} dir={isRTL ? "rtl" : "ltr"}>
        <Typography variant="h6" className={classes.errorStep}>
          {t("ClientProfile.Commande.status.canceled_by_partner")}
        </Typography>
        <Typography className={classes.stepDescription}>
          {t("ClientProfile.Commande.Message.canceled_by_partner")}
        </Typography>
      </div>
    );
  } else if (commandStatus === "Canceled_by_client") {
    return (
      <div className={classes.root} dir={isRTL ? "rtl" : "ltr"}>
        <Typography variant="h6" className={classes.errorStep}>
          {t("ClientProfile.Commande.status.canceled_by_client")}
        </Typography>
        <Typography className={classes.stepDescription}>
          {t("ClientProfile.Commande.Message.canceled_by_client")}
        </Typography>
      </div>
    );
  }
  ...
  return (
    <div className={classes.root} dir={isRTL ? "rtl" : "ltr"} >
      ...
      <div
        ...
        className={`${classes.fullStepper} ${isRTL ? classes.rtlStepper : ""}`}
      >
        <Stepper
        ...
        >
          {statusSteps.map((statusKey, index) => {
            ...
            return (
              <Step  key={statusKey}>
                <StepLabel  style={{paddingLeft:10}} {...labelProps}>
                  {isRTL ? getArabicLabel(status.title) : status.title}
                </StepLabel>
                {index === activeStep && (
      ...
      <div className={classes.mobileStep}>
        <Typography variant="h6" className={classes.mobileStepLabel}>
