...
const ExelentService = ({ afficherBilte = false }) => {
  useEffect(() => {
    AOS.init({
      duration: 1000,
    });
  ...
  return (
    <Content>
      <Middle>
        <Heading>{t("ACCEUILE.PRESTATION-SERVICE-2.title1")}</Heading>
        <HeadingUp>{t("ACCEUILE.PRESTATION-SERVICE-2.title2")}</HeadingUp>
        <Description>{t("ACCEUILE.PRESTATION-SERVICE-2.desc")}</Description>
      </Middle>
